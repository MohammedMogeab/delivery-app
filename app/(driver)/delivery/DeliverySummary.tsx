...
const Stars = ({ value }: { value: number }) => {
  const stars = Array.from({ length: 5 }).map((_, i) => (
    <Text
      key={i}
      className={`${i < value ? "text-[#FBBF24]" : "text-[#475569]"} text-xl mr-1`}
    >
      ★
    </Text>
  ));
      ...
      <ScrollView className="px-4 py-6">
        ...
        <View className="rounded-lg bg-[#0B1220] p-4 mb-4">
          ...
          <View className="flex-row justify-between items-center py-2">
            ...
            <Text className="text-lg text-[#10B981] font-JakartaSemiBold">
              2800 YER
            </Text>
          </View>
        ...
        <View className="rounded-lg bg-[#0B1220] p-4 mb-4">
          ...
          <Text className="text-[#94A3B8] italic mb-3">
            "Driver was polite and on time."
          </Text>

          <Text className="text-white font-JakartaSemiBold mb-3">
            Rate Store Experience (optional)
          </Text>
          <View className="flex-row mb-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <TouchableOpacity
                key={i}
                onPress={() => setStoreRating(i + 1)}
                className="mr-2"
              >
                <Text
                  className={`${i < storeRating ? "text-[#10B981]" : "text-[#475569]"} text-2xl`}
                >
                  ★
                </Text>
              </TouchableOpacity>
