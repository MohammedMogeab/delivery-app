...
const ReasonItem = ({
  ...
  <TouchableOpacity
    ...
    className={`flex-row items-center justify-between rounded-2xl px-4 py-4 mb-3 ${
      selected
        ? "border border-[#0F766E] bg-[#052F26]/20"
        : "border border-[#334155]"
    }`}
  >
    <View className="flex-row items-center">
      <View
        className={`w-5 h-5 mr-3 rounded-full ${selected ? "bg-[#10B981]" : "bg-transparent border border-[#64748B]"}`}
      />
        ...
        <View className="mb-4">
          ...
          <View className="flex-row items-center mb-3">
            <TouchableOpacity
              ...
              className={`w-5 h-5 mr-3 rounded-full ${selectedReason === "Other" ? "bg-[#10B981]" : "bg-transparent border border-[#64748B]"}`}
            />
          ...
          <View className="rounded-lg bg-[#052F26]/20 border border-[#064E3B] p-3 mb-4">
            <Text className="text-sm text-[#9AE6B4]">
              Your report will be reviewed by the admin and store owner. The
              order will be marked as Failed Delivery until resolved.
            </Text>
          </View>
